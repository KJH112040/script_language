...
city = int(input())
city_km = list(map(int,input().split()))
...
for c in L_cost:
    if c != L_cost[len(L_cost)-1]:
        if low_cost > c:
            low_cost = c
            low_cost_loc = L_cost.index(c)

km = 0
for driving in range(city-1):
    if driving >= low_cost_loc:
        km += city_km[driving]
    else:
        total_cost += city_km[driving]*L_cost[driving]
